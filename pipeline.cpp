  ...
  if (m_videoLocation.isEmpty ()) {
    ...
    m_pipeline =
        ...
        ("videotestsrc pattern=0 ! "
            ...
            "glupload ! gleffects effect=9 ! fakesink sync=1", NULL));
  } else {
    QByteArray ba = m_videoLocation.toLocal8Bit ();
    qDebug ("Loading video: %s", ba.data ());
    gchar *pipeline = g_strdup_printf ("uridecodebin name=u ! "
        "videoconvert ! " "fakesink sync=1");
    m_pipeline = GST_PIPELINE (gst_parse_launch (pipeline, NULL));
    GstElement *f = gst_bin_get_by_name (GST_BIN (m_pipeline), "u");
    g_object_set (G_OBJECT (f), "uri", ba.data (), NULL);
  ...
  switch (GST_MESSAGE_TYPE (msg)) {
    case GST_MESSAGE_NEED_CONTEXT:
    {
      ...
      if (g_strcmp0 (context_type, GST_GL_DISPLAY_CONTEXT_TYPE) == 0) {
        ...
        g_print("gst_context_new\n");
        GstContext *display_context = gst_context_new(GST_GL_DISPLAY_CONTEXT_TYPE
                        , TRUE);
        g_print("gst_context_set_gl_display\n");
        gst_context_set_gl_display (display_context, p->display);
        g_print("gst_element_set_context\n");
        ...
        g_print("gst_context_writable_structure\n");
        GstStructure *s = gst_context_writable_structure (app_context);
        g_print("gst_structure_set\n");
